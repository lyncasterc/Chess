  ...
  describe '#get_move' do
    ...
    context 'when the move is valid' do
      ...
      it 'returns the new_pos' do
        new_pos = [2,2]
        expect(game_move.get_move(new_pos, player_piece)).to eq(new_pos)
      end
      ...
      it 'does not output error message' do
        ...
        game_move.get_move(new_pos, player_piece)
      end
      ...
      it 'moves the player piece to the new_pos node' do
        ...
        expect{ game_move.get_move(new_pos, player_piece) }.to change { new_pos_node.piece }.from(nil).to(player_piece)
      end

      it 'changes the player_piece pos instance variable to new_pos' do
        new_pos = [2,2]

        expect { game_move.get_move(new_pos, player_piece) }.to change { player_piece.pos }.from([2,1]).to(new_pos)
      end
      ...
      it 'changes the old node pos instance variable to nil' do
        ...
        expect{ game_move.get_move(new_pos, player_piece) }.to change { old_pos_node.piece }.from(player_piece).to(nil)
      end
      ...
      context 'when the player piece is a pawn' do 
        it 'changes the has_moved instance variable to true' do
          ...
          expect { game_move.get_move(new_pos, player_piece) }.to change { player_piece.has_moved }.from(false).to(true)
        end
      end
      ...
      context 'when the player piece is a rook' do 
        ...
        it 'changes the has_moved instance variable to true' do
          ...
          expect { game_move.get_move(new_pos,rook_player_piece) }.to change { rook_player_piece.has_moved }.from(false).to(true)
        end
      end
